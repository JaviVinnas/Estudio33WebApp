    ...
    <c:choose>
        <c:when test="${empty caja}">
                ...
                <c:forEach var="entry" items="${catalogo}" varStatus="status">
                    <div class="elemento-catalogo">
                        <form method="post" action="TiendaServlet">
                            ...
                            <!-- Precio -->
                            <div>Precio: ${entry.key.precio}&euro;</div>
                            <c:choose>
            ...
            <div id="parte-carrito">
                <c:forEach var="itemCarrito" items="${usuario.carrito}">
                    <div class="elemento-carrito">
                        <form method="post" action="TiendaServlet">
                            ...
                            <!-- Precio -->
                            <div>Precio: ${itemCarrito.key.precio}&euro;</div>
                            <!-- Existencias -->
                ...
                <div>Precio total: ${precio_total}&euro;</div>
                <c:if test="${precio_total ne 0.0}">
                    <form method="post" action="TiendaServlet">
                        <input type="submit" name="pasar_por_caja" value="Pasar por caja">
                    </form>
                </c:if>
            </div>
        </c:when>
        <c:otherwise>
                ...
                <section id="uds_compradas">
                    <c:forEach var="itemCarritoPagar" items="${usuario.carrito}">
                        <c:set var="precioTotalCategoria" scope="session">
                            <fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${itemCarritoPagar.value * itemCarritoPagar.key.precio}" />
                        </c:set>
                        <!-- 10 unidades de "Disco1" a 10€ la unidad = 20€ -->
                        <article class="unidad_comprada">${itemCarritoPagar.value} unidades
                            de ${itemCarritoPagar.key.nombre}
                            a ${itemCarritoPagar.key.precio} &euro; la unidad =
                                ${precioTotalCategoria} &euro;
                        </article>
                    </c:forEach>
                </section>
                <div id="precio_compra">

                    <c:set var="precioTotalMasIva" scope="session">
                        <fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${precio_total * 1.2}" />
                    </c:set>
                    El precio total será ${precio_total}&euro; que con un iva del 20% será ${precioTotalMasIva}&euro;
                </div>
            </div>
            <div id="info_tienda">
                <div>Email: ${email}</div>
            ...
            <form method="post" action="TiendaServlet">
                <input type="submit" name="volver_a_la_tienda" value="Volver a la tienda">
            </form>
        </c:otherwise>
    </c:choose>
